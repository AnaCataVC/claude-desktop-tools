...
## 2.6 Global Config Categories: settings.json, keybindings.json, home CLAUDE.md, and Sanitized MCP Config (2026-09-15)

Discovery only ever walked from the user-chosen `rootPath` downward, so anything that only lives directly under `~/.claude/` (global user `CLAUDE.md`, `settings.json`, `settings.local.json`, `keybindings.json`) was invisible unless `rootPath` happened to include the home directory. Three new categories close that gap: `GlobalSetting`, `Keybinding`, `McpConfig`.

- **`ClaudeConfigDiscoveryService(gitExecutable, homeDirectory)`** takes an optional `homeDirectory` (defaults to `Environment.SpecialFolder.UserProfile`), purely so tests can inject an isolated fake home instead of scanning whatever the real machine's `~/.claude` happens to contain. `DiscoverAsync` always processes `<homeDirectory>/.claude` once, directly -- **not** by seeding the BFS queue with the home directory itself, which would walk every unrelated folder under home (`Documents`, `Downloads`, ...) up to `maxDepth` deep. The existing per-`.claude`-folder collection logic was extracted into `CollectFromDotClaudeDir` so both the BFS loop and this one-off home call share it.
- **`IsJsonConfigAllowed`** mirrors `IsCandidateAllowed`/`IsHookScriptAllowed` (same `SensitiveNameKeywords` filename check, same `HasInfrastructureSecret` content scan) but for `.json` instead of `.md`/`.txt`/script extensions. `CollectSingleFile` is the loose-file equivalent of `CollectCategoryFiles` for `settings.json`, `settings.local.json` and `keybindings.json`, which sit directly under `.claude/` rather than in their own subfolder.
- **MCP server config is extracted and sanitized, never uploaded raw.** On this machine, MCP servers live in `~/.claude.json` (207 KB) alongside `oauthAccount` and the full project/session history -- uploading it as-is would leak far more than intended. `ExtractSanitizedMcpConfig` parses only the root `mcpServers` node and any per-project `projects.<path>.mcpServers`, walks the resulting `JsonNode` tree, and replaces any value whose key name contains `token`/`key`/`secret`/`password`/`auth`/`header`/`env` (case-insensitive) with the literal string `"[REDACTED]"` -- regardless of whether that value is a scalar or a whole object (e.g. an `"env": {...}` block collapses entirely). The result is written to a temp file under the app's own `%LocalAppData%\ClaudeDesktopTools` folder (same directory `LocalSettingsHelper` already uses) and added as a normal `McpConfig` candidate. This redaction is a **heuristic keyed on JSON key names** -- it is not a guarantee against every possible shape of secret, and the first sanitized file uploaded is worth a manual look before trusting the flow.
- **`DriveSyncService.BuildDriveRelativePath` needed no changes** -- its existing `candidate.Category != ClaudeDiscoveryCategory.Context` branch already routes any repo-less, non-Context category into the `_claude-config` bucket. Only the `mimeType` selection in `SyncCandidatesAsync` grew a case for `GlobalSetting`/`Keybinding`/`McpConfig` → `application/json`.

### 2.6.1 Adversarial review found two real leak paths in the first version (2026-09-15)

The first `SanitizeMcpJsonNode` only redacted values sitting directly under a suspicious-looking JSON **key**. An `adversarial-verifier` pass against this session's own live `~/.claude.json` found two concrete bypasses:

1. **Array elements have no key to check.** A stdio MCP server's `args` array commonly carries a bearer token as its own element (`["--header", "Authorization: Bearer <token>"]`, the `mcp-remote` shape) or a connection string with embedded credentials (`postgresql://user:pass@host/db`) as a plain `"url"` value -- neither `"args"` nor `"url"` matches any fragment in `SensitiveMcpKeyFragments`, so both passed through untouched in the original version.
2. **The generated file was the only candidate in the whole pipeline with zero content-level secret scan.** Every other category (`IsCandidateAllowed`, `IsHookScriptAllowed`, `IsJsonConfigAllowed`) gates through `HasInfrastructureSecret`; `ExtractSanitizedMcpConfig`'s output went straight into `directCandidates` with only the key-name heuristic behind it.

**Fix:** `SanitizeMcpJsonNode`'s leaf case (the `default:` branch, reached by every scalar whether under a suspicious key or not) now also content-scans the string value itself against `InfrastructureSecretPatterns` (the existing PEM/AWS/GitHub/Slack set) and a new `McpValueSecretPatterns` list added for value-shaped secrets that have no natural "key" at all: `Bearer\s+[A-Za-z0-9\-_\.=]{16,}` and `://[^/\s:@]+:[^/\s@]+@` (scheme://user:pass@host). On top of that, `ExtractSanitizedMcpConfig` re-scans the *entire* serialized output against both pattern sets before writing anything, and fails closed (returns `false`, writes nothing) if a match survives -- the same posture `IsJsonConfigAllowed` already takes elsewhere (exclude the file rather than upload it partially redacted). This still can't catch every shape (a bare opaque token embedded in a URL *path* segment, with no `Bearer`/`user:pass@` marker, is indistinguishable from a normal path by pattern alone) -- the heuristic-not-a-guarantee caveat in `ExtractSanitizedMcpConfig`'s docstring stands.
