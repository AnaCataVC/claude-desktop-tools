...
- [`claude-process-resource-monitoring-and-control.md`](learning/claude-process-resource-monitoring-and-control.md): Architecture of real-time Claude process monitoring, memory working set trimming, process priority adjustment, high-frequency UI throttling, packaged-app process filtering, and PEB-based working-directory labeling.
- [`unversioned-ai-context-discovery-and-secret-filtering.md`](learning/unversioned-ai-context-discovery-and-secret-filtering.md): Multi-zone BFS context discovery (skills, agents, hooks, references), chunked Git tracking via `git ls-files` (batch size 50), defense-in-depth regex secret scanning, and the discovery view's per-category filter/collapse/bulk-select controls.
