...
- **Unversioned Steering & Context Loss:** Ephemeral instructions, agent rosters, skills, custom hooks, and persistent agent/project memories are easily lost upon workstation failure.
...
6. **Context Discovery & Secret Scanning:** Multi-zone BFS traversal (`CLAUDE.md`, `references/`, `skills/`, `agents/`, `hooks/`, `agent-memory/`, `projects/*/memory/`) with strict exclusion of internal cache/memory directories, batched `git ls-files` checking (chunks of 50), and regex filters for SSH private keys, AWS tokens, GitHub PATs, and Slack secrets.
7. **Cloud Backup with Live Progress, Grouped and Selective:** `DriveSyncService` backs up discovered, unversioned AI context files to Google Drive through an `IProgress<DriveSyncProgress>` callback (per-file percentage, current file name, uploaded/failed counters) with user-cancellable transfers. Orphan files (no owning Git repo) keep their full path relative to the scan root, so files with the same name from different folders land at distinct Drive destinations instead of overwriting each other. The two bucket names those orphans land under (`_sin-repo` for CLAUDE.md/references, `_claude-config` for skills/agents/hooks/agent-memory/project-memory) are configurable in Settings, with a dedicated save button and confirmation message scoped to the Drive card. There is no local change-detection: every untracked candidate is re-uploaded on every sync, and whether Drive ends up overwritten-in-place versus duplicated depends entirely on the receiving Google Apps Script's own upsert-by-path logic (outside this repo). The discovery view groups results by category (CLAUDE.md, Skills, Agents, Scheduled Tasks, Hooks, Memoria de Agente, Memoria de Proyecto) instead of one mixed list, each category collapsible via its own `Expander`, and every file has its own checkbox (checked by default) -- "Sincronizar a Drive" only sends what's still checked, with "Seleccionar todo" / "Ninguno" available both globally and per category. A row of per-category checkboxes lets categories be shown/hidden from the list independently of sync selection (with a "Mostrar todas" reset), a quick action button per category selects only that category's files for sync in one click, already-versioned files are automatically excluded so only unversioned files are presented for sync, and the sync button dynamically reflects the count of selected files with a persisted timestamp of the last synchronization.
...
│── docs/
│   ├── external-references/
│   │   ├── claude-code-memory-architecture.md
...
- **Pérdida de Contexto no Versionado:** Instrucciones personalizadas, habilidades (`skills`), agentes, hooks y memorias persistentes de agente o proyecto que se pierden al formatear el equipo.
...
6. **Descubrimiento de Contexto y Filtro de Secretos:** Exploración BFS de archivos `CLAUDE.md`, referencias, habilidades, agentes, hooks y memorias (`agent-memory/`, `projects/*/memory/`) con verificación Git por lotes de 50 archivos y filtros regex contra claves SSH, AWS, tokens PAT de GitHub y Slack.
7. **Respaldo en la Nube con Progreso en Vivo, Agrupado y Selectivo:** `DriveSyncService` respalda hacia Google Drive los archivos de contexto IA descubiertos y sin versionar, reportando avance por archivo mediante `IProgress<DriveSyncProgress>` (porcentaje, archivo actual, contadores de subidos/fallidos) con cancelación disponible para el usuario. Los archivos huérfanos (sin repo Git) conservan su ruta completa relativa a la raíz del escaneo, por lo que archivos con el mismo nombre en carpetas distintas terminan en destinos distintos de Drive en vez de sobrescribirse entre sí. Las dos carpetas donde caen esos huérfanos (`_sin-repo` para CLAUDE.md/references, `_claude-config` para skills/agentes/hooks/agent-memory/project-memory) son configurables en Ajustes, con un botón de guardado y mensaje de confirmación propios de la tarjeta de Drive. No hay detección de cambios local: cada archivo sin seguimiento se vuelve a subir en cada sincronización, y que Drive termine sobrescribiendo en el mismo lugar o duplicando depende enteramente de la lógica de upsert-por-ruta del Google Apps Script receptor (fuera de este repo). La vista de descubrimiento agrupa los resultados por categoría (CLAUDE.md, Skills, Agentes, Tareas Programadas, Hooks, Memoria de Agente, Memoria de Proyecto) en vez de una sola lista mezclada, cada categoría colapsable mediante su propio `Expander`, y cada archivo tiene su propio checkbox (marcado por defecto) -- "Sincronizar a Drive" solo envía lo que sigue marcado, con "Seleccionar todo" / "Ninguno" disponibles tanto de forma global como por categoría. Una fila de checkboxes por categoría permite mostrar/ocultar categorías de la lista de forma independiente a la selección de sincronización (con un botón "Mostrar todas" para reiniciar el filtro), un botón rápido por categoría selecciona para sincronizar solo los archivos de esa categoría en un clic, los archivos ya versionados en Git se excluyen automáticamente para mostrar únicamente archivos sin seguimiento, y el botón de sincronización cuenta con un contador reactivo dinámico y registro persistido de la última sincronización.
